        ...
        if ($Validatior->fails()) {
            ...
            try {
                ...
                if ($request['banner'] != "") {
                    ...
                    $request->file('banner')->move("uploads/fleet/banner", $name);
                    $data->banner = 'uploads/fleet/banner/'.$name;
                }

                if ($request['image'] != "") {
                    $file1 = $request->file('image');
                    $name1 = rand(11111, 99999) . '.' . $file1->getClientOriginalExtension();
                    $request->file('image')->move("uploads/fleet/image", $name1);
                    $data->image = 'uploads/fleet/image/'.$name1;
                }

                $data->title = $request['title'];
                $data->sub_title = $request['sub_title'];
                $data->description = $request['description'];
                $data->passegers = $request['passegers'];
                $data->luggage_large = $request['luggage_large'];
                $data->luggage_small = $request['luggage_small'];
