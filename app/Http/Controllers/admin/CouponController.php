...
class CouponController extends Controller
{
    public function add(Request $request, $id = Null)
    {
        ...
        $getdata = Coupon::find($decrypted_id);
        if ($id != "") {
            $saveurl = url('admin/coupon/save/' . $id);
            $button = 'Update';
            $page_title = 'Update Coupon';
        } else {
            $saveurl = url('admin/coupon/save');
            $button = 'Add';
    ...
    public function save(Request $request, $id = NUll)
    {
        if (!empty($id)) {
            $decrypted_id  = get_decrypted_value($id, true);
            $data          = Coupon::find($decrypted_id);
            $success_msg   = 'coupon Updated Successfully.';
            $nameValidator = 'required|unique:coupons,title,' . $decrypted_id . ',id,status,1';
        } else {
            $data          = new Coupon;
            $success_msg   = 'coupon Added Successfully.';
            $nameValidator = 'required|unique:coupons,title,3,status';
        }
        ...
        if ($Validatior->fails()) {
            ...
            try {
                if ($request['image'] != "") {
                    ...
                    $request->file('image')->move("uploads/coupon", $name);
                    $data->image = 'uploads/coupon/' . $name;
                }
                $data->title       = $request['title'];
                $data->description = $request['description'];
                $data->discount = $request['discount'];
                $data->coupon_code = $request['coupon_code'];
                $data->offer_use = $request['offer_use'];
        ...
        return redirect()->route('coupon')->withSuccess($success_msg);
    }
    ...
    public function index()
    {
        $data = array(
            'title' => 'View Coupon',
            'page_title' => 'View Coupon',
        );
        return view('admin.coupon.view')->with($data);
    }

    public function anydata(Request $request)
    {
        $anydata = [];
        $anydata = Coupon::orderBy('id', 'DESC')->where('status', '<', 3)->where(function ($query) use ($request) {
        ...
        return Datatables::of($anydata)
            ...
            ->addColumn('action', function ($anydata) {
                ...
                $action = '<a href="' . url('/admin/coupon/add/' . $encrypted_id) . '"><i class="fa fa-edit" title="Edit" data-toggle="tooltip" data-placement="bottom"></i></a> &nbsp;&nbsp;  
    ...
    public function delete(Request $request)
    {
        ...
        if ($data) {
            ...
            $return_arr = array(
                'status' => 'success',
                'message' => 'Coupon Deleted Sussessfully!',
            );
